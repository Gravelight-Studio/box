...
export class ContainerGenerator {
  ...
  async generate(): Promise<void> {
    const containerHandlers = this.handlers.filter(
   ...
   */
  private groupByService(handlers: Handler[]): Record<string, Handler[]> {
    const groups: Record<string, Handler[]> = {};
   ...
   */
  private async generateService(serviceName: string, handlers: Handler[]): Promise<void> {
   ...
   * Generate Dockerfile
   */
  private async generateDockerfile(dir: string, serviceName: string, handlers: Handler[]): Promise<void> {
...
`;
   ...
   * Generate package.json
   */
  private async generatePackageJson(dir: string, serviceName: string): Promise<void> {
    const pkg = {
   ...
   * Generate server.js
   */
  private async generateServerJs(dir: string, handlers: Handler[]): Promise<void> {
...
`;
   ...
   */
  private generateHandlerCode(handler: Handler): string {
...
app.${handler.route.method.toLowerCase()}('${handler.route.path}'${middleware.length > 0 ? `, ${middleware.join(', ')}` : ''}, async (req, res) => {
   ...
   * Generate .dockerignore
   */
  private async generateDockerIgnore(dir: string): Promise<void> {
...
`;
   ...
   * Generate cloudbuild.yaml
   */
  private async generateCloudBuild(dir: string, serviceName: string): Promise<void> {
